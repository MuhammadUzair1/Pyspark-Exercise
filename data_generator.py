...
# Set log level to ERROR to reduce noise in the logs
spark.sparkContext.setLogLevel("ERROR")

# Read products CSV file
products = spark.read.csv(
    "products.csv", header=True, mode="DROPMALFORMED", inferSchema=True
)
products.show()

# Write products as Parquet
products.write.parquet("products_parquet")

# Read sales CSV file
sales = spark.read.csv(
    "sales.csv", header=True, mode="DROPMALFORMED", inferSchema=True
)
sales.show()

# Repartition and write sales as Parquet
sales.repartition(200, col("product_id")).write.parquet("sales_parquet", mode="overwrite")

# Read sellers CSV file
sellers = spark.read.csv(
    "sellers.csv", header=True, mode="DROPMALFORMED", inferSchema=True
)
sellers.show()

# Write sellers as Parquet
sellers.write.parquet("sellers_parquet", mode="overwrite")
